...
void getCodes(Node* encodingTree,string curr, Map<ext_char, string>& codes){
	if (encodingTree->character != NOT_A_CHAR) {
		...
		getCodes(encodingTree->zero, curr+"0", codes);
		getCodes(encodingTree->one, curr+"1", codes);
	}
}

void writeData(string &s, obstream& outfile) {
	for (char c : s) {
		if (c == '1') {
			outfile.writeBit(1);
		}
...
void encodeFile(istream& infile, Node* encodingTree, obstream& outfile) {
	...
	string last = codes[PSEUDO_EOF];
	writeData(last, outfile);
}
...
void decodeFile(ibstream& infile, Node* encodingTree, ostream& file) {
	Node* tempTree = encodingTree;
	while (true) {
		if (tempTree->character == PSEUDO_EOF) {
			break;
		}
		if (tempTree->character != NOT_A_CHAR) {
			file.put(tempTree->character);
			tempTree = encodingTree;
		}
		else {
			int bite = infile.readBit();
			if (bite == 0) {
